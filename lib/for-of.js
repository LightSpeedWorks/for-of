...
function Ranges(from, to, step, incl) {
  ...
  if (!(this instanceof Ranges))
    return new Ranges(from, to, step, incl);
...
Ranges.prototype.next = function next() {
  if (this._index >= this._length)
    return {done: true};

  var obj = {done: false, value: this._value, index: this._index};
...
function next() {
  ...
  if (!self) {
    ...
    _primes[x] = self = new Ranges(len);
    self._this = this;
  }

  var len = self._length || self.length;
  if (len === undefined) len = Math.floor(Number(this));
  if (len === 0) return {done: true};
  ...
  if (typeof self._index === 'undefined') {
...
for (var i of 3)
  for (var j of i)
    console.log('i, j of 3,i: = %s, %s', i, j);
