...
function Range(from, to, step, boundary) {
  ...
  if (to === undefined) boundary = false, to = from, from = 0;
  if (typeof to === 'boolean') boundary = to, to = from, from = boundary? 1: 0;
  if (typeof step === 'boolean') boundary = step, step = undefined;
  ...
  if (!(this instanceof Range))
    return new Range(from, to, step, boundary);

  var len = Math.floor((to - from + (boundary? step: 0))/ step);
...
// 10, 1, -1, true -> 10..1
function range(from, to, step, boundary) {
  var obj = new Range(from, to, step, boundary);
  var len = obj._length;
  if (len <= 0) return [];
